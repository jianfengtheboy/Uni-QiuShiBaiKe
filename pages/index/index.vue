 ...
 * @LastEditTime: 2020-04-06 19:59:53
 * @Description: 
 -->
<template>
		...
		<uni-nav-bar :shadow="false"
			...
			<block slot="left">
				<view class="iconfont icon-qiandao"
							style="font-size: 22px;color: #FF9619;margin-left: 20upx;">
				</view>
			</block>
			<!-- 中间搜索框 -->
			<view style="display: flex;justify-content: center;align-items: center;border-radius: 4px;margin-left: -46upx;height: 60upx;margin-top: 12upx;color: #CCCCCC;background: #F7F7F7;"
						@tap="openSearch">
			...
			<swiper-item v-for="(items, index) in newslist" :key="index"> 
				<scroll-view scroll-y class="list" @scrolltolower="loadmore(index)">
					...
					<template v-else-if="!items.firstload">
						<view style="font-size: 50upx;font-weight: bold;color: #CCCCCC; padding-top: 100upx;"
									class="u-f-ajc">Loading ...
						</view>
					</template>
					<template v-else>
						<!-- 无内容默认 -->
...
export default {
	...
	methods: {
		...
		updateSupport(data) {
			...
			// 操作后的状态
			obj.infonum.index = (data.do == 'ding') ? 1 : 2
			//之前操作过
			if (oldindex !== 0) {
